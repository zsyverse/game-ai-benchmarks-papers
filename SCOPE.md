...
| Game code or project | Executable code, scenes, or engine-project changes that implement game behavior. | 实现游戏行为的可执行代码、场景或引擎工程修改。 |
...
| General infrastructure and runtimes / 通用基础设施与运行时 | Supporting a generator is not the same as proposing one. / 支撑生成器不等于提出生成器。 |
...
| Standalone repair, QA, bug finding, or parameter tuning / 独立修复、QA、查错或调参 | Modifying an existing artifact is insufficient unless it is an internal stage of a substantive generation method. / 只修改既有工件不足以收录；除非它是实质生成方法的内部环节。 |
...
| Isolated assets or components / 孤立资产或组件 | Images, textures, 3D props, music, dialogue, stories, cards, or settlements qualify only when integrated by the paper into an eligible playable output. / 图像、纹理、3D 物件、音乐、对话、故事、卡牌或聚落，只有由论文方法集成进合格可玩产物时才收录。 |
